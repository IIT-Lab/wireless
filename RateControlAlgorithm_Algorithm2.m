...
for numPkt = 1:numPackets 
    ...
    if isempty(y.RxPSDU)
     ...
     %Rate control algorithm - Using fuzzy controller
     % The SNR from the range 5 to 40 is divided into 10 functions of
     % varying intervals and each section is assigned to a MCS
    fis=readfis('RateControlAlgorithm_Algorithm2'); %read the fuzzy rules
    output=evalfis([y.EstimatedSNR],fis);  %choose the best suited algorithm for the given SNR
    cfgVHT.MCS=floor(output);   
end
