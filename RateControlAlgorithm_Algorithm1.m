...
% threshold = [meanSNR-amplitude meanSNR-amplitude+3  meanSNR-amplitude+8 meanSNR-amplitude+9 meanSNR-amplitude+14 meanSNR+amplitude-6 meanSNR+amplitude-4 meanSNR+amplitude-3  meanSNR+amplitude];
...
rcaRelease = 0; % Control the sensitivity when MCS is decreasing
%threshold = [11 14 19 20 25 28 30 31 35]; 
threshold = [meanSNR-amplitude meanSNR-((3/4)*amplitude)  meanSNR-(amplitude/2) meanSNR-(amplitude/4) meanSNR meanSNR+(amplitude/4) meanSNR+(amplitude/3) meanSNR+((3/4)*amplitude)  meanSNR+amplitude];
snrUp = [threshold inf]+rcaAttack;
snrDown = [-inf threshold]-rcaRelease;
snrInd = cfgVHT.MCS; % Store the start MCS value
...
fl=0; %flag to keep track of bit error
...
mfInfo = getfis(fis,'input',1,'mf',3,'params');
  if y.EstimatedSNR>=mfInfo(2)    %check if SNR of packet falls under high SNR range
     if ber(numPkt)~=0            %check for bit error rate
         fl=1 ;                   %update flag
         cfgVHT.MCS=cfgVHT.MCS-1; %decrease MCS by 1
     end
  else 
       fl=0;                      %clear flag
  end
  if fl==0 
    output=evalfis([y.EstimatedSNR ber(numPkt) cfgVHT.MCS],fis); %update MCS according to the Fuzzy rule table
    cfgVHT.MCS=round(output);
  end
end
